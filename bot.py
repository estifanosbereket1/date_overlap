...
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, MessageHandler, CallbackQueryHandler, filters, ContextTypes
from core import DB, process_upload
from dotenv import load_dotenv
...
def record_consent(match_key, chat_id, consent):
    ...
    if not row:
        return None
    if row[0] == chat_id:
        cur.execute("UPDATE consent_requests SET user_a_consent = %s WHERE match_key = %s", (consent, match_key))
    else:
        cur.execute("UPDATE consent_requests SET user_b_consent = %s WHERE match_key = %s", (consent, match_key))
    DB.commit()
    cur.execute("""
        SELECT user_a_chat_id, user_a_handle, user_a_consent,
               user_b_chat_id, user_b_handle, user_b_consent
...
async def my_submissions(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for row in rows:
        ...
        date_str = uploaded_at.strftime("%b %d, %Y")
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton("🗑️ Delete this submission", callback_data=f"delete_{sub_id}")
        ]])
        await update.message.reply_photo(
            photo=io.BytesIO(bytes(photo_bytes)),
            caption=f"Submitted on {date_str}",
...
async def handle_delete(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not row:
        await query.edit_message_caption(caption="❌ Submission not found or already deleted.")
        return

    cur.execute("DELETE FROM submissions WHERE id = %s", (sub_id,))
    DB.commit()

    await query.edit_message_caption(caption="✅ Submission deleted. Good riddance. You deserve better. 👏")